...
function all(func) {
    return function() {
        let parameters = arraySlice.call(arguments);
...
is.empty = function(value) {
    if (is.object(value)) {
        let num = Object.getOwnPropertyNames(value).length;
...
is.dayLightSavingTime = function(obj) {
    let january = new Date(obj.getFullYear(), 0, 1);
    let july = new Date(obj.getFullYear(), 6, 1);
